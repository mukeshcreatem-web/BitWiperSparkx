...
def collapse_all():
    ...
    try:
        ...
        with open(file, "r+b") as f:
            ...
            f.truncate(s)
        text.delete("1.0", END)
        text.insert(END, " ".join("00" for _ in range(min(s, chunk))))
...
def load_gif(path, size=(180, 180)):
    try:
        g = Image.open(path)
        return [ImageTk.PhotoImage(f.copy().resize(size, Image.LANCZOS)) for f in ImageSequence.Iterator(g)]
    except Exception as e:
...
logo_path = resource_path("Spark.png")
if os.path.exists(logo_path):
    try:
        logo_img = Image.open(logo_path)
        logo_img = logo_img.resize((170, 170), Image.LANCZOS)
        logo_photo = ImageTk.PhotoImage(logo_img)
        logo_label = Label(top_panel, image=logo_photo, bg="black")
        logo_label.image = logo_photo
        logo_label.place(x=1300, y=-17)
    except Exception as e:
        print("Error loading spark.png:", e)
else:
    print("spark.png file not found at:", logo_path)
